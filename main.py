...
# 1. Вывод инфо (ФИО) по номеру документа
...
# 3. добавит новый документ в каталог и в перечень полок, спросив его номер, тип, имя владельца и номер полки,
...
# 7 Удаление документа по номеру
def remove_doc_from_shelf(doc_number):
    for directory_number, directory_docs_list in directories.items():
        if doc_number in directory_docs_list:
            ...
            return directories[directory_number]
